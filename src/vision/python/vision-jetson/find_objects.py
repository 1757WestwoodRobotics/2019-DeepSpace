...
def generate_color_table(object_type):
    ...
    while (index<256):
        ...
        if (object_type == constant.TYPE_REFLECTIVE_TAPE):
            ...
            color_1 = int(index * 0 + 85) # Unbuntu land calls this bule
            color_2 = int(index * 0 + 245)
            color_3 = int(index * 0 + 245)
        else:
...
def match_pixel_to_object(color,object_type):
    ...
    target=reflective_tape_color_table[color[1]]
    if ((70 < color[0] < 100) and (220 < color[1] < 256) and (220 < color[2] < 256)):
#    if ((220 < color[0] < 256) and (70 < color[1] < 100) and (220 < color[2] < 256)):
#    if ((220 < color[0] < 256) and (220 < color[1] < 100) and (70 < color[2] < 100)):
   	fit_distance = 10
...
def search_for_objects(picture_in, acceleration, animate, objects_to_find):
 ...
 #   original_rows, original_cols, layers = picture_in.shape
    ...
    working_picture=cv2.bilateralFilter(working_picture,3,50,50)
    ...
    if run_fast:
        ...
        for row in range (constant.SEARCH_RADIUS, center_row, 1):
   ...
   #  show_picture("HSV", mask, 10)
    ...
    if run_fast:
        ...
        object_list = find_objects(mask,constant.SEARCH_RADIUS, animate)
...
def this_is_it(camera_number,acceleration_factor,ip_address):
    ...
    parent_table, sub_table = init_network_tables(ip_address)
    ...
    cap=configure_camera(camera_number,True)
...
# network table ip adress
this_is_it(0,1,'10.17.57.2')
