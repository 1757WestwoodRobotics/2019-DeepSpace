...
# image format ? YUYV
    ...
    # to get a list of v4l2 commands type in Unbuntu v4l2-ctl --help-all
    # check what frame sizes are supplorted Ubuntu 
    if robot_execution:
       ...
       subprocess.call("v4l2-ctl -d " + str(camera_number) + " --set-ctrl brightness=30", shell=True)
       subprocess.call("v4l2-ctl -d " + str(camera_number) + " --set-ctrl saturation=100", shell=True)
       subprocess.call("v4l2-ctl -d " + str(camera_number) + " --set-ctrl exposure_auto=1",shell=True)
       subprocess.call("v4l2-ctl -d " + str(camera_number) + " --set-ctrl exposure_absolute=20", shell=True)
       subprocess.call("v4l2-ctl -d " + str(camera_number) + " --set-ctrl contrast=5", shell=True)
    ...
    time.sleep(1) # give it a second to configure the camera.
    return cap
    ...
    while run_again:
        ...
        for row in range(45,rows):
            for col in range(0,cols):
                ...
                cv2.imshow("location", working)
                cv2.waitKey(100)
                print("row " + str(row) + " col " + str(col) + " value ")
                print(picture[row, col])
